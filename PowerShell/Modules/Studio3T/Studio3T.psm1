...
function Studio3T {
    ...
    $selectedContainer = Read-SpectreSelection `
        -Message 'Select a Mongo container' `
        -Choices $mongoContainers `
        -ChoiceLabelProperty { "$($_.PortMappings[0].ExternalPort) => $($_.Name)" } `
        -EnableSearch `
        -Color 'Cyan1'
