<#
.SYNOPSIS
    Manages selected Windows Terminal settings.
.DESCRIPTION
    Reads the Windows Terminal settings file and supports color-scheme operations.
#>
param (
    [string] $verb,
    [string] $arg0 = ""
...
function ReadSettingsFile
{
    ...
    $defaultSettingsPath = FindDefaultSettingsFile

    if ($defaultSettingsPath)
    {
        $defaultSettings = ReadJsonFile $defaultSettingsPath
        $schemesByName = @{}

        foreach ($scheme in @($defaultSettings.schemes) + @($settings.schemes))
        {
            if ($scheme.name)
            {
                # User schemes are processed last and override defaults with the same name.
                $schemesByName[$scheme.name] = $scheme
            }
        }

        $settings | Add-Member -MemberType NoteProperty -Name schemes -Value @($schemesByName.Values) -Force
    }
...
function ReadJsonFile
{
    ...
    $lines = Get-Content -LiteralPath $path | Where-Object {!$_.TrimStart().StartsWith("//")}
    $json = [String]::Join("", $lines)
    return ConvertFrom-Json $json
}

function FindDefaultSettingsFile
{
    $package = Get-AppxPackage -Name Microsoft.WindowsTerminal -ErrorAction SilentlyContinue
    if ($package.InstallLocation)
    {
        $packageDefaults = Join-Path $package.InstallLocation 'defaults.json'
        if (Test-Path -LiteralPath $packageDefaults)
        {
            return $packageDefaults
        }
    }

    $unpackagedDefaults = Join-Path $env:LOCALAPPDATA 'Microsoft\Windows Terminal\defaults.json'
    if (Test-Path -LiteralPath $unpackagedDefaults)
    {
        return $unpackagedDefaults
    }
