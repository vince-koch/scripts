...
.DESCRIPTION
    Selects or accepts a target machine and supports fullscreen and multi-monitor sessions.
#>
param (
...
if ($options -eq $null) {
    ...
    if ($files.Length -gt 0) {
        ...
        $choice = Read-SpectreSelection `
            -Message 'Select an RDP file' `
            -Choices $files `
            -ChoiceLabelProperty { [System.IO.Path]::GetFileNameWithoutExtension($_) } `
            -EnableSearch `
            -Color 'Cyan1'
        if ($choice -ne $null) {
