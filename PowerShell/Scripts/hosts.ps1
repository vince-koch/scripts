<#
.SYNOPSIS
    Manages entries in the Windows hosts file.
.DESCRIPTION
    Provides elevated commands for inspecting and modifying local hostname mappings.
#>
param (
    [string] $verb,
    [string] $arg0 = ""
...
if (-not (Get-Module -Name Elevate)) {
    Import-Module (Join-Path $PSScriptRoot 'Modules\Elevate\Elevate.psm1') -DisableNameChecking -Force
}
